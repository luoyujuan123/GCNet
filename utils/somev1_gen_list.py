...
if __name__ == '__main__':
    ...
    with open('somethingv1_category.txt', 'w') as f:
        f.write('\n'.join(categories))
    ...
    for (filename_input, filename_output) in zip(files_input, files_output):
        ...
        for i in range(len(folders)):
            ...
            dir_files = os.listdir(os.path.join('/media/sdc/liujiayu/datasets/20bn-something-something-v1', curFolder))
            output.append('%s %d %d' % ('/media/sdc/liujiayu/datasets/20bn-something-something-v1/' + curFolder, len(dir_files), curIDX))
