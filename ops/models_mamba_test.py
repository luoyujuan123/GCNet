...
class TSN(nn.Module):
    ...
    def forward(self, input, target=None, no_reshape=False):
        ...
        # 兼容旧调用: model(input, True/False)
        if isinstance(target, bool) and no_reshape is False:
            no_reshape = target
            target = None

        # 训练/验证日志里会读取loss_con.item()，这里先提供稳定的零损失占位
        ...
        if not no_reshape:
            ...
            #     temperature=0.07
            # )
            #loss = self.contrastive_classification_loss(out[0], out[1], targets[0], targets[1])
