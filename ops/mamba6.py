...
from ops.multiheadselfattention import MultiHeadAttention,SelfAttention,MLP,SHSA,SEBlock
...
class ResidualBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.a=0.7
        self.b=0.3
        self.attention = SelfAttention(dropout=0.8)
        self.saha=SHSA(8)
        self.conv1 = nn.Conv1d(in_channels=config.d_model,out_channels=config.d_model,kernel_size=1)
    ...
    def forward(self, x):
        ...
        #改：
        # rx =x.transpose(1, 2) # (B, ED, L)
        # rx=self.conv1(rx)
        # rx=self.norm(rx.transpose(1,2))
        #output = self.mixer(self.norm(self.attention(x,x,x))) +x
        #output = self.mixer(self.norm(self.attention(x,x,x))) +x
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.SEBlock=SEBlock(in_channels=8)

        

        self.config = config

        self.mlp = MLP(12544,12544)
    ...
    def forward(self, x):
        ...
        x, z = xz.chunk(2, dim=-1) # (B, L, ED), (B, L, ED)(4,8,12444)
